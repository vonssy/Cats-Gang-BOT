...
class Cats:
    ...
    def welcome(self):
        print(
            f"""
        {Fore.GREEN + Style.BRIGHT}Auto Claim {Fore.BLUE + Style.BRIGHT}Cats Gang - BOT [Task]
            """
            f"""
        ...
        for attempt in range(5):
            try:
                ...
                print(
                    ...
                    f"{Fore.RED + Style.BRIGHT}[ Failed to Connect to Server ]{Style.RESET_ALL}"
                    f"{Fore.YELLOW + Style.BRIGHT} Retrying...{Style.RESET_ALL}"
                    f"{Fore.WHITE + Style.BRIGHT}[ {attempt + 1}/{5} ]{Style.RESET_ALL}",
                    end="\r",
                    flush=True
                )
    ...
    async def complete_tasks(self, query: str, task_id: str):
        ...
        try:
            ...
            self.log(f"{Fore.YELLOW+Style.BRIGHT}[ Disconnected From the Server ] : {e}{Style.RESET_ALL}")
    ...
    async def process_query(self, query: str):
        try:
            ...
            if user:
                ...
                self.log(f"{Fore.YELLOW+Style.BRIGHT}[ Failed to Fetching Account ]{Style.RESET_ALL}")
            ...
            if tasks:
                ...
                if manual_task:
                    self.log(f"{Fore.YELLOW+Style.BRIGHT}[ Remaining Manual Task ]{Style.RESET_ALL}")
            else:
                self.log(f"{Fore.YELLOW+Style.BRIGHT}[ Failed to Fetching Task ]{Style.RESET_ALL}")
...
if __name__ == "__main__":
    try:
        ...
        print(
            f"{Fore.CYAN + Style.BRIGHT}[ {datetime.now().astimezone(wib).strftime('%x %X %Z')} ]{Style.RESET_ALL}"
            f"{Fore.WHITE + Style.BRIGHT} | {Style.RESET_ALL}"
            f"{Fore.RED + Style.BRIGHT}[ EXIT ] Cats Gang - BOT{Style.RESET_ALL}",                                       
        )